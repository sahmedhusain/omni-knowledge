...
export const Layout: React.FC<LayoutProps> = ({
  ...
  const [role, setRole] = React.useState<'reader' | 'admin'>('admin');
  ...
  const filteredMenuItems = menuItems.filter(item => {
    if (role === 'reader' && (item.id === 'admin' || item.id === 'metrics')) {
      return false;
    }
    return true;
  });
  ...
  return (
    <div className="w-screen h-screen flex flex-row overflow-hidden text-slate-100 font-sans">
      ...
      <aside className="w-64 h-full flex-shrink-0 flex flex-col justify-between p-6 border-r border-white/[0.04] bg-slate-950/20 backdrop-blur-3xl">
            ...
            {filteredMenuItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`group flex flex-col gap-0.5 px-4 py-3 rounded-xl transition-all duration-200 text-left w-full relative cursor-pointer ${
                    isActive
        ...
        {/* Live Backend indicators & Roles */}
        <div className="p-4 rounded-xl bg-white/[0.01] border border-white/[0.03] flex flex-col gap-3">
          {/* Active Role configuration */}
          <div className="flex items-center justify-between text-[11px] font-bold text-slate-400 pb-2.5 border-b border-white/[0.03]">
            <span>Active Role</span>
            <select
              value={role}
              onChange={(e) => {
                const newRole = e.target.value as 'reader' | 'admin';
                setRole(newRole);
                if (newRole === 'reader') {
                  setActivePage('search');
                }
              }}
              className="bg-slate-900 border border-slate-800 text-slate-200 rounded-lg text-[9px] font-extrabold uppercase px-2 py-1 outline-none cursor-pointer"
            >
